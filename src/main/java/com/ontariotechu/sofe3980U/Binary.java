...
public class Binary {
	...
	public Binary(String number) {
		...
		this.number = number; // exclude the trailing zeros if any
	...
	public static Binary or(Binary num1, Binary num2) {
		...
		// pad the shorter number with leading zeroes
		if (len1 > len2) {
		...
		{
			if (ind1 >= 0) { // if num1 has a digit to add
				c1 = (num1.number.charAt(ind1) == '1') ? 1 : 0;
				ind1--; // update ind1
			}
			if (ind2 >= 0) { // if num2 has a digit to add
				c2 = (num2.number.charAt(ind2) == '1') ? 1 : 0;
				ind2--; // update ind2
			}
			if (c1 == 1 || c2 == 1) {
				num3 = '1' + num3;
